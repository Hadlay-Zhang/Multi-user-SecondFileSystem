...
File *OpenFileTable::FAlloc()
{
	...
	printf("[error] OpenFileTable::FAlloc: No Free File Struct\n");
	u.u_error = ENFILE;
	return NULL;
}
...
Inode *InodeTable::IGet(int inumber)
{
	...
	while (true)
	{
		...
		if (index >= 0) /* 找到内存拷贝 */
		{
			...
			/* 多用户new：如果该内存Inode被上锁 */
            pthread_mutex_lock(&pInode->mutex_inode);
		...
		else /* 没有Inode的内存拷贝，则分配一个空闲内存Inode */
		{
			...
			if (NULL == pInode)
			{
				printf("[error] InodeTable::IGet: Inode Table Overflow !\n");
				u.u_error = ENFILE;
				return NULL;
			}
			else /* 分配空闲Inode成功，将外存Inode读入新分配的内存Inode */
			{
				...
				{
					printf("[error] InodeTable::IGet: I/O Error !\n");
					/* 释放缓存 */
					bm.Brelse(pBuf);
					/* 释放占据的内存Inode */
					this->IPut(pInode);
