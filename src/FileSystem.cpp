...
void FileSystem::LoadSuperBlock()
{
    ...
    {
        printf("[ERROR]: In FileSystem::LoadSuperBlock: 载入超级块失败.\n");
    }
	g_sb.s_ronly = 0;
    // 最近一次更新时间设置
    time_t cur_time;
...
void FileSystem::Update()
{
	...
	{
		printf("[INFO]: In FileSystem::Update: SuperBlock无需更新.\n");
		return;
	}
    // 多用户new：任何时刻只允许一个进程进行超级块写回操作，因此上锁
    pthread_mutex_lock(&sb->s_ilock);
    pthread_mutex_lock(&sb->s_flock);
    printf("[INFO]: In FileSystem::Update: s_flock s_ilock上锁成功.\n");
	sb->s_fmod = 0;
	/* 写入SuperBlock最后存访时间 */
	time_t cur_time;
	time(&cur_time);
    ...
    // 多用户new：完成超级块写回操作，解锁互斥锁
    pthread_mutex_unlock(&sb->s_flock);
    pthread_mutex_unlock(&sb->s_ilock);
    printf("[INFO]: In FileSystem::Update: s_flock s_ilock 解锁成功.\n");
    ...
    printf("[INFO]: In FileSystem::Update: SuperBlock更新完毕.\n");
}
...
Inode *FileSystem::IAlloc()
{
	...
	if (sb->s_ninode <= 0)
	{
        // 多用户new：将空闲Inode索引表互斥锁上锁
        pthread_mutex_lock(&sb->s_ilock);
        printf("[INFO]: In FileSystem::IAlloc: s_ilock 上锁成功.\n");
        ...
        // 多用户new：将空闲Inode索引表互斥锁解锁
        pthread_mutex_unlock(&sb->s_ilock);
        printf("[INFO]: In FileSystem::IAlloc: s_ilock 解锁成功.\n");
...
Buf *FileSystem::Alloc()
{
    ...
    // 多用户new：分配空闲磁盘块，任何时刻只允许一个用户进程修改空闲盘块索引表
    pthread_mutex_lock(&sb->s_flock);
    printf("[INFO]: In FileSystem::Alloc: s_flock 上锁成功.\n");
	...
	/* 普通情况下成功分配到一空闲磁盘块 */
    // 多用户new：分配空闲磁盘块成功，解锁空闲盘块索引表互斥锁
    pthread_mutex_unlock(&sb->s_flock);
    printf("[INFO]: In FileSystem::Alloc: s_flock 解锁成功.\n");
...
void FileSystem::Free(int blkno)
{
	 ...
	 */
	sb->s_fmod = 1;
    // 多用户new：释放盘块，空闲盘块索引表上锁
    pthread_mutex_lock(&sb->s_flock);
    printf("[INFO]: In FileSystem::Free: s_flock 上锁成功.\n");
	...
	sb->s_fmod = 1;
    // 多用户new：释放盘块完成，空闲盘块索引表解锁
    pthread_mutex_unlock(&sb->s_flock);
    printf("[INFO]: In FileSystem::Free: s_flock 解锁成功.\n");
}
