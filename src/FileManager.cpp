...
void FileManager::Creat()
{
	...
	if (NULL == pInode)
	{
		if (u.u_error)
		{
			printf("[error] FileManager::Create: u_error;\n");
			return;
		}
		/* 创建Inode */
	...
	// printf("[info] FileManager::Creat: creat file success;\n");
}
...
void FileManager::Open1(Inode *pInode, int mode, int trf)
{
	...
	if (u.u_error)
	{
		printf("[error] FileManager::Open1: u_error;\n");
...
void FileManager::Rdwr(enum File::FileFlags mode)
{
	...
	/* 权限检测，比对入口参数mode和File结构的f_flags */
	/* 读写的模式不正确 */
...
{
	// printf("[info] FileManger::NameI: start\n");
	Inode *pInode;
	Buf *pBuf;
	char curchar;
	char *pChar;
	...
	while (true)
	{
		...
		if (u.u_error != NOERROR)
		{
			printf("[error] FileManager::NameI: u.u_error\n");
			break; /* goto out; */
		}
		...
		while (true)
		{
			...
			memcpy((int *)&u.u_dent, src, sizeof(DirectoryEntry));
...
{
	// printf("[info] FileManager::SetCurDir: pathname: %s\n", pathname);
	User &u = Kernel::Instance().GetUser();
...
void FileManager::ChDir()
{
	...
	if (NULL == pInode)
	{
		printf("[error] FileManager::ChDir: No such path\n");
		return;
	}
	/* 搜索到的文件不是目录文件 */
	if ((pInode->i_mode & Inode::IFMT) != Inode::IFDIR)
	{
		printf("[error] FileManager::ChDir: Not a directory\n");
		u.u_error = ENOTDIR;
		this->m_InodeTable->IPut(pInode);
		return;
	}
	if (this->Access(pInode, Inode::IEXEC))
	{
		printf("[error] FileManager::ChDir: No permission\n");
		this->m_InodeTable->IPut(pInode);
		return;
	}
	...
	// char pathname[512] = {0};
	// memcpy(pathname, &u.u_arg[0], sizeof(unsigned long long));
...
void FileManager::Link()
{
    ...
    pthread_mutex_unlock(&pInode->mutex_inode);
	...
	{
		printf("[error] FileManager::Link: u_error\n");
		/* 出错，释放资源并退出 */
		this->m_InodeTable->IPut(pInode);
		return;
	}
...
void FileManager::UnLink()
{
	...
	{
		printf("[error] FileManager::UnLink: No such file or directory\n");
		return;
	}
	// pDeleteInode->Prele();
	...
	if (NULL == pInode)
	{
		printf("[error] FileManager::UnLink: IGet failed\n");
	}
	/* 只有root可以unlink目录文件 */
	if ((pInode->i_mode & Inode::IFMT) == Inode::IFDIR && !u.SUser())
	{
		...
		this->m_InodeTable->IPut(pInode);
		printf("[error] FileManager::UnLink: Permission denied\n");
		return;
	}
	/* 写入清零后的目录项 */
	u.u_IOParam.m_Offset -= (DirectoryEntry::DIRSIZ + 4);
